 ...
 var GUID = WebUploader.Base.guid();//一个GUID
        ...
        function initUpload() {
            ...
            var uploader = WebUploader.create({
                ...
                // 文件接收服务端。
                server: '/SystemSet/SysFiles/Upload',
                ...
                accept: {
                    title: 'intoTypes',
                    extensions: 'rar,zip,doc,xls,docx,xlsx,pdf,jpg,jpeg,png,gif',
                    mimeTypes: '.rar,.zip,.doc,.xls,.docx,.xlsx,.pdf,.jpg,.jpeg,.png,.gif'
                }
            ...
            uploader.on('uploadSuccess', function (file, response) {
                ...
                $.post('/SystemSet/SysFiles/Merge', { guid: GUID, fileName: file.name, fileType: fileType }, function (data) {
                    ...
                    $li.find('span.download').html("<a href=\"/SystemSet/SysFiles/DownFile?filePath=" + data.filepath + "&amp;fileName=" + data.filename + "\">下载</a>")
                    $li.find('span.webuploadDelbtn').show();
                    $li.find('span.filepath').html(data.filepath);
                    //增加列表存储
                    files.push(data);
                    if (top.$('#ifrm_Edit').length > 0) {
                        top.$('#ifrm_Edit')[0].contentWindow.appendFile(data);
                    }
                    if (currentPage().appendFile && typeof (currentPage().appendFile) == "function") {
                        currentPage().appendFile(data);
                    }
                });
            });
